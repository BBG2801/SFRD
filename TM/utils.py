...
def get_dataset(dataset, data_path, batch_size=1, subset="imagenette", args=None):
    ...
    if dataset == 'MNIST':
        ...
        print("DEBUG abs_data_path:", os.path.abspath(data_path))
        print("DEBUG cwd          :", os.getcwd())

        devkit_path = os.path.join(data_path, "ILSVRC2012_devkit_t12.tar.gz")
        train_tar   = os.path.join(data_path, "ILSVRC2012_img_train.tar")
        val_tar     = os.path.join(data_path, "ILSVRC2012_img_val.tar")

        print("DEBUG devkit_path  :", devkit_path)
        print("DEBUG train_tar    :", train_tar)
        print("DEBUG val_tar      :", val_tar)

        print("DEBUG devkit exists:", os.path.exists(devkit_path))
        print("DEBUG train exists :", os.path.exists(train_tar))
        print("DEBUG val exists   :", os.path.exists(val_tar))

        if os.path.exists(data_path):
            print("DEBUG root files   :", sorted(os.listdir(data_path))[:20])
        else:
            print("DEBUG data_path does not exist!")
...
def rand_crop(x, param):
    # x: [B, C, H, W]
    assert x.dim() == 4, f"rand_crop expects 4D input, got shape={x.shape}"
    ...
    grid_x = (grid_x + translation_x + 1).clamp(0, H + 1).long()
    grid_y = (grid_y + translation_y + 1).clamp(0, W + 1).long()
    grid_batch = grid_batch.long()

    x_pad = x_pad.permute(0, 2, 3, 1).contiguous()   # [B, H+2, W+2, C]
    out = x_pad[grid_batch, grid_x, grid_y]          # [B, H, W, C]
    out = out.permute(0, 3, 1, 2).contiguous()       # [B, C, H, W]
    return out

def rand_cutout(x, param):
    # x: [B, C, H, W]
    assert x.dim() == 4, f"rand_cutout expects 4D input, got shape={x.shape}"
    ...
    cutout_h = max(1, min(cutout_h, H))
    cutout_w = max(1, min(cutout_w, W))
    ...
    grid_x = (grid_x + offset_x - cutout_h // 2).clamp(0, H - 1).long()
    grid_y = (grid_y + offset_y - cutout_w // 2).clamp(0, W - 1).long()
    grid_batch = grid_batch.long()

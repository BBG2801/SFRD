"""
Video inter-class relation distillation for SFRD-style codebases.

Uses a frozen ConvNet3D embedder from the current Video `networks.py`.
"""
...
class FrozenVideoFeatureExtractor(nn.Module):
    """
    Frozen ConvNet3D.embed wrapper.
    ...
    def __init__(
        ...
        net_act: str = "relu",
        net_norm: str = "none",
        net_pooling: str = "maxpooling",
        checkpoint_path: Optional[str] = None,
    ) -> None:
        super().__init__()
        self.backbone = ConvNet3D(
            channel=channel,
...
def build_frozen_video_extractor(
    channel: int,
    num_classes: int,
    im_size: Tuple[int, int],
    frames: int,
    checkpoint_path: Optional[str] = None,
) -> FrozenVideoFeatureExtractor:
    return FrozenVideoFeatureExtractor(
        channel=channel,
        num_classes=num_classes,
        im_size=im_size,
        frames=frames,
...
def sample_balanced_real_batch(
    ...
    for c, cls_indices in enumerate(indices_class):
        ...
        x = video_all[chosen].to(device, non_blocking=True)
...
def compute_class_centroids(feats: Tensor, labels: Tensor, num_classes: int) -> Tensor:
    centroids = []
...
class InterClassRelationDistillationLossVideo(nn.Module):
    def __init__(
        self,
    ...
    def _relation_loss_from_centroids(self, mu_real: Tensor, mu_syn: Tensor):
    ...
    def forward_from_sfrd(
        ...
        with torch.no_grad():
            ...
            mu_real = compute_class_centroids(feat_real, real_y, num_classes=self.num_classes)
